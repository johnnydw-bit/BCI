...
function buildCommitteeGuide() {
  const children = [
    ...
    h2('6. The spreadsheet view'),
    p('The triage report uses a compact spreadsheet layout — one row per submission. Click any row to open a side panel with full detail and all management actions.'),
    h3('Column order'),
    stdTable(
      ['Column', 'What it shows'],
      [
        ['Score', 'AI score (or override score) with colour-coded band'],
        ['Improvement', 'AI summary or original description'],
        ['Area', 'Category the submission belongs to'],
        ['Impl', 'Implementation complexity: quick win, project, or programme'],
        ['Cost', '🎯 confirmed cost target · ❓ AI estimated range'],
        ['Date', '🎯 confirmed target date · ❓ AI estimated date · submitted date if neither'],
        ['Decision', 'Current status — dropdown for Club Manager'],
        ['Owner', 'Assigned owner role — dropdown for Club Manager'],
        ['Flags', 'Icon flags: ⚠ H&S, ⚡ Quick win, 💰 Revenue, 🔁 Recurring, etc.'],
      ]
    ),
    ...
    tipBox('Target date and cost target can be set independently at any time from the Committee Decision section of the side panel — they do not require a status change.'),
    ...
    h2('9. Target dates and cost targets'),
    h3('Target date'),
    pRuns([{ text: 'AI estimated date (❓): ', bold: true }, { text: 'Calculated from the AI\'s implementation time estimate; shown as a reference in grey.' }]),
    pRuns([{ text: 'Confirmed target date (🎯): ', bold: true }, { text: 'Set by the Club Manager in the Committee Decision panel. Shown in green with a ✓ confirmed indicator. Visible to the member in their My Improvements page.' }]),
    p('Use the "Use AI estimate" shortcut to pre-fill the AI suggested date, then adjust if needed. The date can be cleared with the ✕ button.'),
    h3('Cost target'),
    pRuns([{ text: 'AI estimated cost (❓): ', bold: true }, { text: 'The AI provides a cost range based on typical costs for similar work. Shown in grey as a reference.' }]),
    pRuns([{ text: 'Confirmed cost target (🎯): ', bold: true }, { text: 'Enter a specific budget figure (whole pounds) in the Cost target field in the Committee Decision panel. Replaces the AI estimate with a green 🎯 confirmed figure in the spreadsheet.' }]),
    p('Use the "Use AI midpoint" shortcut to pre-fill the midpoint of the AI range as a starting point. The cost target can be cleared with the ✕ button.'),
    infoBox('Cost target is a planning figure for the committee — it is not currently shown to members.'),
